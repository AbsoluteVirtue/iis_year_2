...
#include "Movie.h" // без этого нельзя
...
{
	// зачем параметр ammount?
	// ввод и добавление памяти надо разделять 
	int desiredNumber = 0;
...
void searchMovie(Movie *&Obj, const int ammount)
{
	...
	// ввод и вывод вынести за функцию
...
void addMovieToTheEnd(Movie *&Obj, int &ammount)
{
	...
	if (ammount == 0) // проверять нужно сам указатель, а не стороннюю переменную
	{
		// проверка не обязательна, delete можно вызвать на пустой указатель
		Obj = tempObj;
	}
	else
	{
...
{
	ofstream fout("labrab6.txt"); // не стоит хард-кодить данные
	for (int i = 0; i < ammount; i++)
	{
...
void readListFromTxt(Movie *&Obj, int &ammount)
{
	...
	// не стоит открывать файл два раза, лучше передвинуть курсор в начало
