...
void Flight_controller::Kb_arr_enter()
{
	...
	for (int i = 0; i < num_of_flights; i++)
	{
		...
		cout << " Depature time: ";
		cin >> &fl_arr[i].dep_time;
		cout << " Arrival time: ";
		cin >> &fl_arr[i].arv_time;
		cout << " Ticket cost: ";
		cin >> fl_arr[i].tic_cost;
	}
}
...
{
	// неправильная логика, добавляет в конец текущего массива независимо от того, было ли добавлено ранее
	num_of_flights++;
...
void Flight_controller::File_create()
{
	...
	for (int i = 0; i < num_of_flights; i++)
		OutputFile << fl_arr[i].number << " " << fl_arr[i].dep_city << " " << fl_arr[i].dep_time << " " << fl_arr[i].arv_city << " " << fl_arr[i].arv_time << " " << fl_arr[i].tic_cost << " ;" << endl;
}
